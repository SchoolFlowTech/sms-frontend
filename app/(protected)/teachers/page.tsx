...
export default function StudentsPage() {
  ...
  async function fetchTeachers() {
    try {
      ...
      if (res.data.errors) {
        throw new Error(
          res.data.errors[0]?.message || "Failed to fetch teachers"
        );
      }
  ...
  const filteredTeachers = useMemo(() => {
    ...
    return teachers.filter((s) => {
      ...
      // ✅ Format joining date SAME way as display
      const formattedJoiningDate = s.joiningDate
        ? new Date(Number(s.joiningDate))
          .toLocaleDateString("en-IN")
          .toLowerCase()
        : "";

      const id = String(s.teacherId ?? "").toLowerCase();
      ...
      return (
        ...
        id.includes(term) ||
        formattedJoiningDate.includes(term) ||
                ...
                return (
                  ...
                  >
                      ...
                      <div className="flex items-center gap-3">
                        ...
                        <div>
                          ...
                          <p className="text-xs text-gray-500">
                            ID: {teacher.teacherId ?? "—"} | Experience:{" "}
                            {teacher.experience ?? "—"}
                          </p>
                        </div>
                      </div>
                    ...
                    <td className="py-4 px-6 text-gray-600">
                      {teacher.joiningDate
                        ? new Date(Number(teacher.joiningDate)).toLocaleDateString("en-IN")
                        : "N/A"}
                    </td>
                    <td className="py-4 px-6 text-gray-600">
